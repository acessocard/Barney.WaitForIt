...
namespace Barney.WaitForIt
{
    internal class RootInitializer
    {
        ...
        {
            _logger.LogInformation("Waiting for async initialization");

            try
            {
                foreach (var initializer in _initializers)
                {
                    _logger.LogInformation("Waiting for async initialization for {InitializerType}", initializer.GetType());
                    try
                    {
